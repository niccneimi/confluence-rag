...
agent = create_agent(
    ...
    system_prompt=(
        ...
        "Сначала вызови qdrant_tool, затем ответь корректно и подробно"
        "Дополняй запрос пользователя ключевыми словами для точного поиска"
        "Ответ должен содержать точные данные из базы данных"
        "Используй ссылки на страницы по каждому из упоминаемых фактов"
        "Если данных не найдено, то ответь: данных нет"
    )
)

if __name__ == '__main__':
    result = agent.invoke({
        "messages": [{'role': 'user', 'content': 'Загрузка URL в список прмокодов. Пример валидного и не валидного URL'}]
    })
